...
// =================== GAME SCRIPT ===================
// jsLines pattern - no template literals wrapping this block
...
function buildGameScript(sprites) {
  ...
  L.push("  const nb=getEl('nameBox'), gb=getEl('gunBar'), ob=getEl('gameOver'), si=getEl('scoreInfo');");
  ...
  L.push("  if(si) si.style.display=gameState==='gameover'?'block':'none';");
  ...
  L.push("    ctx.fillStyle=e.hp/e.maxHp>0.5?'#4f4':'#f44';ctx.fillRect(e.x,e.y-7,e.w*(e.hp/e.maxHp),5);");
  L.push("  }");
  ...
  L.push("      text('TAP',W-28,y+40,13,can?'#fff':'#444','right');");
  L.push("    });");
  L.push("  }");
  ...
  L.push("    leaderboardData.slice(0,10).forEach(function(r,i){");
  L.push("      const col=i===0?'#ffdd00':i<3?'#aaa':'#444';");
  L.push("      text((i+1)+'. '+r.player_name+'   '+r.score+'   W'+r.wave_reached,W/2,182+i*24,13,col,'center');");
  L.push("    });");
  L.push("  } else {");
  L.push("    text('Type your name below and tap PLAY',W/2,200,14,'#555','center');");
  L.push("  }");
  L.push("}");

  // Game over - now shown in HTML overlay
  L.push("function showGameOverUI(){");
  L.push("  const acc=stats.totalShots>0?Math.round(stats.totalHits/stats.totalShots*100):0;");
  L.push("  const el=getEl('scoreInfo');");
  L.push("  if(!el) return;");
  L.push("  el.innerHTML='<b style=color:#ffdd00>Score: '+score+'</b>  Wave: '+wave+'<br>'");
  L.push("    +'Accuracy: '+acc+'%  |  Pixels moved: '+Math.round(stats.pixelsMoved)+'<br>'");
  L.push("    +'Panic shots: '+stats.panicShots+'  |  Best combo: '+stats.longestCombo+'<br>'");
  L.push("    +'Boss kills: '+stats.bossKills+'  |  Near misses: '+stats.nearMisses;");
  L.push("}");
  ...
  L.push("        gunStats[b.gun].hits++;gunStats[b.gun].damage+=b.dmg;");
  L.push("        stats.totalHits++;stats.combo++;");
  L.push("        if(stats.combo>stats.longestCombo) stats.longestCombo=stats.combo;");
  L.push("        if(b.gun==='rocket'){enemies.forEach(function(n){if(n!==e&&n.hp>0&&Math.abs(n.x-e.x)<70&&Math.abs(n.y-e.y)<70){n.hp-=b.dmg*0.5;}});spawnPart(e.x+e.w/2,e.y+e.h/2,'#ff4400',14);}");
  L.push("        if(b.gun==='lightning'){const nb=enemies.find(function(n){return n!==e&&n.hp>0&&Math.abs(n.x-e.x)<110;});if(nb){nb.hp-=b.dmg*0.65;}}");
  L.push("        if(b.gun==='blackhole'){enemies.forEach(function(n){if(n.hp>0){const dx=b.x-n.x,dy=b.y-n.y,dist=Math.sqrt(dx*dx+dy*dy)||1;if(dist<150){n.x+=dx/dist*6;n.y+=dy/dist*6;}}});}");
  L.push("        if(e.hp<=0){gunStats[b.gun].kills++;score+=e.creditValue;credits+=e.creditValue;if(e.isBoss)stats.bossKills++;if(!enemyKills[e.type])enemyKills[e.type]=0;enemyKills[e.type]++;spawnPart(e.x+e.w/2,e.y+e.h/2,e.isBoss?'#ff0000':'#ffaa00',e.isBoss?30:8);}");
  ...
  L.push("      playerHP-=e.isBoss?30:15;stats.cleanThisWave=false;e.hp=0;");
  L.push("      spawnPart(playerX+20,playerY+20,'#ff0000',14);");
  L.push("      if(playerHP<=0){endGame('hit');}");
  L.push("    }");
  L.push("    if(e.y+e.h>playerY+60&&e.hp>0){stats.nearMisses++;stats.cleanThisWave=false;e.y=-e.h-10;}");
  L.push("  });");
  ...
  L.push("  const inp=getEl('nameInput');");
  L.push("  if(inp) playerName=inp.value.trim();");
  L.push("  if(!playerName){if(inp){inp.focus();inp.style.borderColor='#ff4444';}return;}");
  ...
  L.push("  stats={pixelsMoved:0,panicShots:0,gunSwitches:0,longestCombo:0,combo:0,nearMisses:0,totalShots:0,totalHits:0,shotsMissed:0,bossKills:0,consecutiveWavesClean:0,cleanThisWave:true};");
  ...
  L.push("  setUI();");
  L.push("  showGameOverUI();");
  L.push("  if(!sessionId) return;");
  L.push("  const dur=Date.now()-gameStartTime;");
  L.push("  const acc=stats.totalShots>0?Math.round(stats.totalHits/stats.totalShots*100):0;");
  L.push("  const ws=[");
  L.push("    {name:'accuracy_pct',value:acc},{name:'pixels_traveled',value:Math.round(stats.pixelsMoved)},");
  L.push("    {name:'panic_shots',value:stats.panicShots},{name:'longest_combo',value:stats.longestCombo},");
  L.push("    {name:'gun_switches',value:stats.gunSwitches},{name:'near_misses',value:stats.nearMisses},");
  L.push("    {name:'boss_kills',value:stats.bossKills},{name:'credits_wasted',value:credits},");
  L.push("    {name:'shots_missed',value:stats.shotsMissed},{name:'consecutive_clean_waves',value:stats.consecutiveWavesClean}");
  L.push("  ];");
  ...
  L.push("function playAgain(){");
  L.push("  gameState='menu';");
  L.push("  setUI();");
  L.push("  loadLeaderboard();");
  L.push("}");
  ...
  L.push("  if(gameState==='gameover') return;");
  L.push("  if(showShop){");
  ...
  L.push("  if(touchStartX!==null){const dx=tx-touchStartX;playerX=Math.max(0,Math.min(W-42,playerX+dx*0.7));touchStartX=tx;stats.pixelsMoved+=Math.abs(dx*0.7);}");
  L.push("},{ passive:false });");

  // Click on canvas (desktop fallback)
  ...
  L.push("    if(e.key==='ArrowLeft'||e.key==='a') playerX=Math.max(0,playerX-8);");
  L.push("    if(e.key==='ArrowRight'||e.key==='d') playerX=Math.min(W-42,playerX+8);");
  L.push("    if(e.key==='q'||e.key==='Q') switchGun(-1);");
  L.push("    if(e.key==='e'||e.key==='E') switchGun(1);");
  L.push("  }");
  L.push("  if(showShop){");
  L.push("    if(e.key===' ') spawnWave();");
  L.push("    const n=parseInt(e.key);if(!isNaN(n)&&shopItems[n-1]) buyGun(shopItems[n-1].id);");
  L.push("  }");
  L.push("  if(gameState==='gameover'&&e.key==='Enter') playAgain();");
  L.push("});");

  // Auto-fire
  L.push("setInterval(function(){if(gameState==='playing'&&!showShop) shoot();},50);");

  // Draw loop - fix the infinite recursion bug
  ...
  L.push("    text('GAME OVER',W/2,H/2-20,36,'#ff4444','center');");
  L.push("    return;");
  L.push("  }");
...
function buildAdminHTML(spriteList) {
  ...
  const parts = [
    ...
    "input[type=file]{color:#aaa;padding:8px;border:1px solid #333;background:#111;width:100%;margin-bottom:10px;font-size:16px;}",
    "button.upload{background:#004400;color:#00ff88;border:1px solid #00ff88;padding:12px 24px;cursor:pointer;font-family:monospace;font-size:16px;border-radius:4px;width:100%;}",
    ...
    "<p style='font-size:12px;color:#555;margin-bottom:12px;'>Tap 'Choose File' to pick images from your photo library. They become enemies immediately after upload.</p>",
    "<div id='msg'></div>",
    "<input type='file' id='file' accept='image/*' multiple>",
    "<button class='upload' onclick='upload()'>Upload to R2</button>",
    "<h2>Current Sprites (" + spriteList.length + ")</h2>",
    spriteList.length === 0 ? "<p style='color:#444;font-size:13px;'>No sprites yet. Upload images above.</p>" : "",
    "<table>" + rows + "</table>",
    "<script>",
    "function msg(t,ok){const d=document.getElementById('msg');d.textContent=t;d.className=ok?'ok':'er';d.style.display='block';setTimeout(function(){d.style.display='none';},4000);}",
    "function upload(){",
    ...
    "    fetch('/admin/upload',{method:'POST',body:form}).then(function(r){return r.json();}).then(function(d){done++;if(done===f.length){msg('Uploaded '+done+' file(s) - refresh game to see them!',true);}}).catch(function(e){msg('Failed: '+e,false);});",
    "  });",
    "}",
    "function del(key){if(!confirm('Delete '+key+'?')) return;fetch('/admin/sprite/'+encodeURIComponent(key),{method:'DELETE'}).then(function(r){return r.json();}).then(function(d){if(d.ok){location.reload();}else{alert('Error: '+d.error);}}).catch(function(){alert('Delete failed');});}",
    "</script>",
    "</body></html>"
  ];
  return parts.join("
...
async function apiEndSession(env, req) {
  ...
  await env.DB.prepare("UPDATE sessions SET score=?,wave_reached=?,duration_ms=?,credits_earned=?,credits_wasted=?,ended_at=datetime('now'),cause_of_death=? WHERE id=?")
    .bind(sc, wr, dur, sc, cw, safe(cause_of_death || "unknown"), session_id).run();
  if (gun_stats && typeof gun_stats === "object") {
    for (const [gid, gs] of Object.entries(gun_stats)) {
      const g = gs || {};
      await env.DB.prepare("INSERT INTO gun_stats (session_id,gun_type,shots_fired,shots_hit,damage_dealt,kills) VALUES (?,?,?,?,?,?) ON CONFLICT(session_id,gun_type) DO UPDATE SET shots_fired=excluded.shots_fired,shots_hit=excluded.shots_hit,damage_dealt=excluded.damage_dealt,kills=excluded.kills")
        .bind(session_id, safe(gid), Number(g.shots)||0, Number(g.hits)||0, Number(g.damage)||0, Number(g.kills)||0).run();
    }
  }
  if (enemy_kills && typeof enemy_kills === "object") {
    for (const [type, count] of Object.entries(enemy_kills)) {
      await env.DB.prepare("INSERT INTO enemy_kills (session_id,enemy_type,kill_count) VALUES (?,?,?) ON CONFLICT(session_id,enemy_type) DO UPDATE SET kill_count=excluded.kill_count")
        .bind(session_id, safe(type), Number(count)||0).run();
    }
  }
  if (Array.isArray(upgrade_log)) {
    for (const u of upgrade_log) {
      await env.DB.prepare("INSERT INTO upgrade_log (session_id,gun_id,cost,wave_number) VALUES (?,?,?,?)")
        .bind(session_id, safe(u.gunId||""), Number(u.cost)||0, Number(u.wave)||0).run();
    }
  }
  if (Array.isArray(weird_stats)) {
    for (const ws of weird_stats) {
      await env.DB.prepare("INSERT INTO weird_stats (session_id,stat_name,stat_value) VALUES (?,?,?)")
        .bind(session_id, safe(ws.name||""), Number(ws.value)||0).run();
    }
  }
  if (sc > 0) {
    await env.DB.prepare("INSERT INTO leaderboard (player_name,score,wave_reached,duration_ms,session_id) VALUES (?,?,?,?,?) ON CONFLICT(session_id) DO UPDATE SET score=excluded.score,wave_reached=excluded.wave_reached,duration_ms=excluded.duration_ms")
      .bind(safe(body.player_name||""), sc, wr, dur, session_id).run();
  }
  return j({ ok: true });
}
