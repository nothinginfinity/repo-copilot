...
const R_GALAXY_FLOOR = 1500, R_GALAXY_CEIL = 4500, R_GALAXY_K = 171;
function galaxySphereRadius(domainCount){
  return Math.max(R_GALAXY_FLOOR, Math.min(R_GALAXY_CEIL, R_GALAXY_K*Math.sqrt(domainCount)));
}
...
function layoutLinks(links){
  ...
  const rGalaxy=galaxySphereRadius(domains.length);
  const anchors={};
  domains.forEach(function(d,i){const p=fibPoint(i,domains.length,rGalaxy);anchors[d]={x:p.x,y:p.y,z:p.z,count:groups[d].length};});
...
function buildGameHTML(layout){
  ...
  const parts=[
    ...
    "#adPromptSponsored{color:#fc6;font-size:10px;font-weight:bold;text-align:center;letter-spacing:1px;text-transform:uppercase;margin-bottom:6px;}",
    ...
    "  <div id='adPromptCard'>",
    "    <div id='adPromptSponsored'>\uD83D\uDCE2 Sponsored</div>",
...
function buildAdminHTML(links){
  ...
  const parts=[
    ...
    "<a href='/admin/review-queue' style='margin-left:14px;'>\uD83D\uDEE1\uFE0F Review Queue</a>",
...
// Review queue: the practical stand-in for "advertiser approval" until real advertiser
// accounts exist (out of v3.0.5's scope) - a single operator surface listing every
// pending_review variant system-wide with quality score, guardrail flags, and an
// Approve/Reject action per item, writing to ad_variant_reviews.
function buildReviewQueueHTML(items){
  const rows=items.map(function(it){
    let flags=[];
    try{flags=JSON.parse(it.flags_json||"[]");}catch(e){}
    const flagsHtml=flags.length?("<div class='rqFlags'>"+flags.map(function(f){return "<span class='rqFlag "+(f.type==="claim_hard_block"?"rqFlagHard":"rqFlagSoft")+"'>"+safe(f.type)+"</span>";}).join("")+"</div>"):"";
    return "<div class='rqCard'>"+
      "<div class='rqTop'><b>"+safe(it.variant_type||"variant")+"</b><span class='rqScore'>quality: "+(it.quality_score!=null?it.quality_score:"\u2014")+"</span></div>"+
      "<div class='rqTitle'>"+(it.title?safe(it.title):"<i>no proposed title</i>")+"</div>"+
      (it.copy?("<div class='rqCopy'>"+safe(it.copy)+"</div>"):"")+
      "<div class='rqRationale'>Rationale: "+safe(it.rationale||"")+"</div>"+
      flagsHtml+
      "<textarea class='rqNotes' id='notes_"+safe(it.id)+"' placeholder='Review notes (optional)'></textarea>"+
      "<div class='rqBtnRow'>"+
      "<button class='rqBtn rqApprove' onclick=\"reviewVariant('"+safe(it.id)+"','approved')\">\u2705 Approve</button>"+
      "<button class='rqBtn rqReject' onclick=\"reviewVariant('"+safe(it.id)+"','rejected')\">\u274C Reject</button>"+
      "</div></div>";
  }).join("");
  const parts=[
    "<!DOCTYPE html><html lang='en'><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1'>",
    "<title>Link Lane - Review Queue</title>",
    "<style>body{background:#06040c;color:#aaa;font-family:monospace;padding:20px;max-width:600px;margin:0 auto;}h1{color:#00ff88;font-size:20px;margin-bottom:4px;}a.back{color:#00ff88;display:inline-block;margin-bottom:14px;}.rqCard{background:#0a0814;border:1px solid #1a1a2a;border-radius:6px;padding:12px;margin-bottom:10px;}.rqTop{display:flex;justify-content:space-between;color:#ffdd00;font-size:12px;margin-bottom:6px;}.rqScore{color:#6cf;}.rqTitle{color:#ddd;font-size:13px;margin-bottom:4px;}.rqCopy{color:#889;font-size:12px;margin-bottom:6px;}.rqRationale{color:#667;font-size:11px;margin-bottom:8px;}.rqFlags{margin-bottom:8px;}.rqFlag{display:inline-block;padding:2px 6px;border-radius:3px;font-size:9px;margin-right:4px;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.5px;}.rqFlagHard{background:#330000;color:#f66;border:1px solid #600;}.rqFlagSoft{background:#332200;color:#fc6;border:1px solid #660;}.rqNotes{width:100%;background:#000;color:#ccc;border:1px solid #1a1a2a;border-radius:4px;padding:8px;font-family:monospace;font-size:12px;min-height:44px;box-sizing:border-box;margin-bottom:8px;}.rqBtnRow{display:flex;gap:8px;}.rqBtn{flex:1;padding:9px;border-radius:5px;font-family:monospace;font-size:12px;font-weight:bold;cursor:pointer;border:none;}.rqApprove{background:#003300;color:#4f4;border:1px solid #4f4;}.rqReject{background:#330000;color:#f66;border:1px solid #f66;}.empty{color:#333;font-size:13px;}</style>",
    "</head><body>",
    "<a class='back' href='/admin'>\u2190 admin</a>",
    "<h1>\uD83D\uDEE1\uFE0F Review Queue ("+items.length+")</h1>",
    items.length?rows:"<p class='empty'>Nothing pending review.</p>",
    "<script>",
    "async function reviewVariant(id,decision){",
    "  const notes=document.getElementById('notes_'+id).value.trim();",
    "  if(!confirm((decision==='approved'?'Approve':'Reject')+' this variant?'))return;",
    "  const r=await fetch('/api/ad-variants/'+id+'/review',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({decision:decision,notes:notes})});",
    "  const d=await r.json();",
...
async function apiCaptureAdEntity(env,req){
  ...
  if(variant_id){
    ...
    if(variant){
      // Abuse guard: the same released variant respawns 4s after each capture, which would let
      // someone farm repeated contributor-reward grants by re-shooting it back-to-back. Cap
      // reward grants for a given variant to at most one per REWARD_COOLDOWN_MS window,
      // regardless of who captures it - the capture itself is still logged either way.
      const lastReward=await env.DB.prepare("SELECT created_at FROM ad_contributor_rewards WHERE variant_id=? ORDER BY created_at DESC LIMIT 1").bind(safe(variant_id)).first();
      const cooledDown=!lastReward||(new Date(now).getTime()-new Date(lastReward.created_at).getTime())>=REWARD_COOLDOWN_MS;
      if(cooledDown){
  ...
  return j({ok:true,id,status,title:(creative&&creative.title)||null,reward_value:reward.value||null,reward_granted:variant_id?rewardGranted:null});
}
...
async function apiImproveAd(env,id,req){
  ...
  const existingCount=await env.DB.prepare("SELECT COUNT(*) as c FROM ad_variants WHERE source_capture_id=?").bind(safe(id)).first();
  if(existingCount&&existingCount.c>=MAX_VARIANTS_PER_CAPTURE) return j({ok:false,error:"too many proposed improvements for this capture ("+existingCount.c+"/"+MAX_VARIANTS_PER_CAPTURE+") \u2014 submit or clean up existing drafts first"},400);
...
// Submit a draft variant for review: runs the automated guardrail scan (claims/quality),
// then either auto-rejects hard-blocked content or moves the variant into pending_review
// for a human decision via apiReviewVariant.
async function apiSubmitVariant(env,id){
  const variant=await env.DB.prepare("SELECT id,title,copy,status FROM ad_variants WHERE id=?").bind(safe(id)).first();
  if(!variant) return j({ok:false,error:"not found"},404);
  if(variant.status!=="draft") return j({ok:false,error:"only a draft variant can be submitted for review (current status: "+variant.status+")"},400);
  const check=runPolicyChecks(variant.title,variant.copy);
  const now=new Date().toISOString();
  await env.DB.prepare("INSERT INTO ad_variant_policy_checks (id,variant_id,quality_score,flags_json,recommendation,created_at) VALUES (?,?,?,?,?,?)")
    .bind(uid(),safe(id),check.quality_score,JSON.stringify(check.flags),check.recommendation,now).run();
  if(check.hard_block){
    await env.DB.prepare("UPDATE ad_variants SET status='rejected' WHERE id=?").bind(safe(id)).run();
    await env.DB.prepare("INSERT INTO ad_variant_reviews (id,variant_id,reviewer_type,status,notes,created_at) VALUES (?,?,?,?,?,?)")
      .bind(uid(),safe(id),"auto","rejected","Automatically rejected: content matched a disallowed claim pattern ("+check.flags.map(function(f){return f.type;}).join(", ")+").",now).run();
    return j({ok:true,status:"rejected",auto:true,quality_score:check.quality_score,flags:check.flags});
  }
  await env.DB.prepare("UPDATE ad_variants SET status='pending_review' WHERE id=?").bind(safe(id)).run();
  return j({ok:true,status:"pending_review",quality_score:check.quality_score,flags:check.flags});
}

// Human (currently: the single operator/admin surface - no separate advertiser accounts exist
// yet, that's out of this version's scope) decision on a pending_review variant.
async function apiReviewVariant(env,id,req){
  const variant=await env.DB.prepare("SELECT id,status FROM ad_variants WHERE id=?").bind(safe(id)).first();
  if(!variant) return j({ok:false,error:"not found"},404);
  if(variant.status!=="pending_review") return j({ok:false,error:"only a pending_review variant can be reviewed (current status: "+variant.status+")"},400);
  const body=await req.json().catch(()=>({}));
  const {decision,notes}=body;
  if(decision!=="approved"&&decision!=="rejected") return j({ok:false,error:"decision must be 'approved' or 'rejected'"},400);
  const now=new Date().toISOString();
  await env.DB.prepare("UPDATE ad_variants SET status=? WHERE id=?").bind(decision,safe(id)).run();
  await env.DB.prepare("INSERT INTO ad_variant_reviews (id,variant_id,reviewer_type,status,notes,created_at) VALUES (?,?,?,?,?,?)")
    .bind(uid(),safe(id),"admin",decision,notes||null,now).run();
  return j({ok:true,status:decision});
}

// Pull a live released variant out of circulation (e.g. a real advertiser wants it stopped,
// or it's underperforming). Retired variants no longer appear in apiListReleasedVariants
// (its query already filters on status='released') and so stop spawning on next page load.
async function apiRetireVariant(env,id){
  const variant=await env.DB.prepare("SELECT id,status FROM ad_variants WHERE id=?").bind(safe(id)).first();
  if(!variant) return j({ok:false,error:"not found"},404);
  if(variant.status!=="released") return j({ok:false,error:"only a released variant can be retired (current status: "+variant.status+")"},400);
  await env.DB.prepare("UPDATE ad_variants SET status='retired' WHERE id=?").bind(safe(id)).run();
  return j({ok:true,status:"retired"});
}

async function apiReleaseVariant(env,id,req){
  const variant=await env.DB.prepare("SELECT id,status,creator_user_id FROM ad_variants WHERE id=?").bind(safe(id)).first();
  if(!variant) return j({ok:false,error:"not found"},404);
  const body=await req.json().catch(()=>({}));
  const demoMode=body&&body.demo_mode===true;
  if(variant.status==="approved"){
    // real approval path - always allowed
  }else if(demoMode&&(variant.status==="draft"||variant.status==="pending_review")){
    // demo_mode explicitly bypasses the approval requirement, but only from draft/pending_review -
    // it can never rescue a rejected or retired variant, and can't be used on an already-released one.
  }else{
    return j({ok:false,error:"only an approved variant can be released (current status: "+variant.status+"). Pass demo_mode:true to bypass approval for testing from draft/pending_review."},400);
  }
  const lastReview=await env.DB.prepare("SELECT id,status,notes,created_at FROM ad_variant_reviews WHERE variant_id=? AND status='approved' ORDER BY created_at DESC LIMIT 1").bind(safe(id)).first();
  const now=new Date().toISOString();
  const provenance=lastReview?{review_id:lastReview.id,reviewed_at:lastReview.created_at,path:"approved"}:{path:demoMode?"demo_mode_bypass":"unknown"};
  ...
  await env.DB.prepare("INSERT INTO ad_release_events (id,variant_id,released_by_user_id,release_pool,status,release_json,created_at) VALUES (?,?,?,?,?,?,?)")
    .bind(uid(),safe(id),variant.creator_user_id||"user_device","demo","released",JSON.stringify(provenance),now).run();
  return j({ok:true,status:"released",provenance:provenance});
}
...
function buildBountyVaultHTML(items){
  const rows=items.map(function(it){
    return "<a class='cardRow' href='/bounty/"+safe(it.id)+"'>"+
      ...
      "<div class='cardMeta'>"+safe(it.reward_value||it.reward_type||"")+" \u00B7 <span style='color:#fc6;'>Sponsored</span></div></div>"+
...
function buildBountyDetailHTML(item,rewards,variants){
  ...
  const variantsHtml=variants.map(function(v){
    let actionHtml="";
    if(v.status==="draft") actionHtml="<button class='btnHalf btnWatch' style='margin-top:8px;width:100%;' onclick=\"submitVariant('"+safe(v.id)+"')\">\uD83D\uDCE4 Submit for Review</button>";
    else if(v.status==="pending_review") actionHtml="<div class='noteDate' style='margin-top:8px;color:#ffdd00;'>\u23F3 Pending review</div>";
    else if(v.status==="approved") actionHtml="<button class='btnHalf btnWatch' style='margin-top:8px;width:100%;' onclick=\"releaseVariant('"+safe(v.id)+"')\">\uD83D\uDE80 Release</button>";
    else if(v.status==="rejected") actionHtml="<div class='noteDate' style='margin-top:8px;color:#f66;'>\u274C Rejected"+(v.review_notes?(": "+safe(v.review_notes)):"")+"</div>";
    else if(v.status==="released") actionHtml="<div class='noteDate' style='margin-top:8px;color:#6cf;'>\u2600\uFE0F Live in space</div><button class='btnHalf btnDiscard' style='margin-top:6px;width:100%;' onclick=\"retireVariant('"+safe(v.id)+"')\">\uD83D\uDED1 Retire</button>";
    else if(v.status==="retired") actionHtml="<div class='noteDate' style='margin-top:8px;color:#667;'>\uD83D\uDED1 Retired</div>";
    return "<div class='noteRow'><div class='noteText'><b>"+safe(v.variant_type||"")+"</b>"+(v.title?(" \u2014 "+safe(v.title)):"")+"<br>"+safe(v.rationale||"")+"</div><div class='noteDate'>"+safe(v.status)+" \u00B7 "+safe((v.created_at||"").slice(0,16).replace("T"," "))+"</div>"+actionHtml+"</div>";
  }).join("");
  const parts=[
    "<!DOCTYPE html><html lang='en'><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1'>",
    ...
    "<div class='bvBadge' style='background:#33220022;color:#fc6;border:1px solid #663;display:inline-block;margin-bottom:8px;'>\uD83D\uDCE2 Sponsored Content</div>",
    ...
    "  const r=await fetch('/api/ad-variants/'+id+'/release',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({})});",
    "  const d=await r.json();",
    "  if(d.ok)location.reload();else alert(d.error||'Failed');",
    "}",
    "async function submitVariant(id){",
    "  if(!confirm('Submit this improvement for review?'))return;",
    "  const r=await fetch('/api/ad-variants/'+id+'/submit',{method:'POST'});",
    "  const d=await r.json();",
    "  if(d.ok){alert(d.auto?('Automatically rejected: '+(d.flags&&d.flags.length?d.flags.map(function(f){return f.type;}).join(', '):'policy check failed')):'Submitted for review.');location.reload();}else{alert(d.error||'Failed');}",
    "}",
    "async function retireVariant(id){",
    "  if(!confirm('Retire this variant? It will stop appearing/spawning.'))return;",
    "  const r=await fetch('/api/ad-variants/'+id+'/retire',{method:'POST'});",
    "  const d=await r.json();",
    "  if(d.ok)location.reload();else alert(d.error||'Failed');",
    "}",
...
export default {
  async fetch(request,env){
    ...
    if(path==="/admin"&&method==="GET"){
      const r=await env.DB.prepare("SELECT id,url,title,domain,og_image_key,group_name,is_short FROM links ORDER BY added_at DESC LIMIT 5000").all();
      return new Response(buildAdminHTML(r.results||[]),{headers:{"Content-Type":"text/html;charset=UTF-8"}});
    }
    if(path==="/admin/review-queue"&&method==="GET"){
      const r=await env.DB.prepare("SELECT v.id,v.variant_type,v.title,v.copy,v.status,v.created_at,c.rationale,pc.quality_score,pc.flags_json FROM ad_variants v LEFT JOIN ad_variant_changes c ON c.variant_id=v.id LEFT JOIN ad_variant_policy_checks pc ON pc.variant_id=v.id WHERE v.status='pending_review' ORDER BY v.created_at ASC").all();
      return new Response(buildReviewQueueHTML(r.results||[]),{headers:{"Content-Type":"text/html;charset=UTF-8"}});
    }
    ...
    if(/^\/api\/ad-variants\/[^/]+\/release$/.test(path)&&method==="POST") return apiReleaseVariant(env,path.split("/")[3],request);
    if(/^\/api\/ad-variants\/[^/]+\/submit$/.test(path)&&method==="POST") return apiSubmitVariant(env,path.split("/")[3]);
    if(/^\/api\/ad-variants\/[^/]+\/review$/.test(path)&&method==="POST") return apiReviewVariant(env,path.split("/")[3],request);
    if(/^\/api\/ad-variants\/[^/]+\/retire$/.test(path)&&method==="POST") return apiRetireVariant(env,path.split("/")[3]);
    ...
    if(path.startsWith("/bounty/")&&method==="GET"){
      ...
      const variants=await env.DB.prepare("SELECT v.id,v.variant_type,v.title,v.copy,v.status,v.created_at,c.rationale,(SELECT notes FROM ad_variant_reviews r WHERE r.variant_id=v.id ORDER BY r.created_at DESC LIMIT 1) AS review_notes FROM ad_variants v LEFT JOIN ad_variant_changes c ON c.variant_id=v.id WHERE v.source_capture_id=? ORDER BY v.created_at DESC").bind(safe(bId)).all();
    ...
    if(path==="/"||path===""){
      const r=await env.DB.prepare("SELECT id,url,title,description,domain,og_image_key,group_name,is_short,published_at,added_at FROM links ORDER BY COALESCE(group_name,domain), added_at LIMIT 5000").all();
